...
function entityFor(cat,park,name){const aliases={"harry potter and the battle at the ministry":["ministry of magic","ministry"]},target=n(name),targets=[target,...(aliases[target]||[])],es=Object.values(cat?.entities||{}).filter(x=>x.parkName===park),names=x=>[x.queueTimes?.name,x.themeParksWiki?.name,x.name].filter(Boolean).map(n);return es.find(x=>names(x).some(v=>targets.includes(v)))||es.find(x=>names(x).some(v=>targets.some(t=>v.includes(t)||t.includes(v))))||null}
...
function nextReference(cat,park){const cards=[...document.querySelectorAll(".trip-item")].filter(c=>!["complete","skip"].includes(c.dataset.status));for(const card of cards){const id=card.dataset.catalogId||card.dataset.itemId,entity=cat.entities?.[id]||Object.values(cat.entities||{}).find(x=>x.itineraryId===card.dataset.itemId&&(!park||x.parkName===park)),location=validLoc(entity);if(location)return{location,label:referenceLabel(entity),source:"next-stop"}}const fallback=Object.values(cat.entities||{}).find(x=>(!park||x.parkName===park)&&validLoc(x));return fallback?{location:validLoc(fallback),label:referenceLabel(fallback),source:"park-fallback"}:null}function referenceLabel(entity){return entity?.name?.replace(/ priorities$/i,"")||"mapped park location"}
...
function getReference(bar,cat,park,request,done){const status=bar.querySelector(".wait-tool-status"),useNext=message=>{const ref=nextReference(cat,park);status.textContent=ref?`${message}: ${ref.label}.`:"No mapped itinerary reference is available.";done(ref)};if(bar.dataset.ref!=="current"){useNext("Distances use next mapped stop");return}const cached=currentReference();if(cached){status.textContent="Distances use recent phone location.";done(cached);return}if(!request){useNext("Tap Current location to enable GPS; temporarily using");return}if(!navigator.geolocation){useNext("GPS unavailable; using");return}status.textContent="Getting phone location…";navigator.geolocation.getCurrentPosition(pos=>{const v={latitude:pos.coords.latitude,longitude:pos.coords.longitude,timestamp:Date.now()};sessionStorage.setItem("vaca-current-location",JSON.stringify(v));status.textContent="Distances use current phone location.";done({location:v,label:"current location",source:"gps"})},()=>useNext("Location unavailable; using"),{enableHighAccuracy:true,timeout:10000,maximumAge:300000})}
...
const target=panel.querySelector("[data-live-waits]"),park=pagePark(panel),aux=await loadWaitAux();
let toolbar;const draw=()=>getReference(toolbar,aux.catalog,park,requestGps,reference=>{const rows=priority.map(c=>{const r=ride(parks[String(c.parkId)]?.rides||[],c.name),meta=ratingMeta(aux.ratings,park,c.name),entity=entityFor(aux.catalog,park,c.name),location=validLoc(entity);let meters=null;if(reference&&location)meters=haversine(reference.location,location);return{c,r,meta,meters,reference}});const sort=toolbar.dataset.sort;rows.sort((a,b)=>{const badA=a.meta?.group==="not-recommended"?1:0,badB=b.meta?.group==="not-recommended"?1:0;if(badA!==badB)return badA-badB;if(sort==="distance")return(a.meters??Infinity)-(b.meters??Infinity)||waitValue(a.r)-waitValue(b.r);if(sort==="wait")return waitValue(a.r)-waitValue(b.r)||(b.meta?.rating||0)-(a.meta?.rating||0);return(b.meta?.rating||-1)-(a.meta?.rating||-1)||(a.meters??Infinity)-(b.meters??Infinity)||waitValue(a.r)-waitValue(b.r)});target.innerHTML=`<div class="live-rides integrated-live-rides">${rows.map(({c,r,meta,meters,reference})=>{const status=!r?"Unavailable":r.isOpen?`${r.waitMinutes} min`:"Closed",rating=meta?.rating?`<span class="wait-family-rating ${meta.group==="not-recommended"?"not-recommended":""}">${meta.group==="not-recommended"?"🚫 ":""}${meta.rating}⭐</span>`:"",travel=meters!=null?`<small class="wait-distance">🚶 ${walkText(meters)} · ${distanceText(meters)}${reference?.label?` from ${e(reference.label)}`:""}</small>`:`<small class="wait-distance unavailable">Distance unavailable</small>`;return`<div class="live-ride" data-family-rating="${meta?.rating||""}" data-distance="${meters??""}"><div class="live-ride-copy"><div class="live-ride-name-line">${rating}<strong class="live-ride-name">${e(c.name)}</strong></div>${travel}</div>${r?.queueTimesUrl?`<a href="${r.queueTimesUrl}" target="_blank"><span class="live-wait ${cls(r)}">${e(status)}</span></a>`:`<span class="live-wait closed">${e(status)}</span>`}</div>`}).join("")}</div>`});
toolbar=waitToolbar(panel,park,()=>draw());draw()}
