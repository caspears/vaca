...
function entityFor(cat,park,name){const target=n(name),es=Object.values(cat?.entities||{}).filter(x=>x.parkName===park);return es.find(x=>[x.queueTimes?.name,x.themeParksWiki?.name,x.name].some(v=>n(v)===target))||es.find(x=>[x.queueTimes?.name,x.themeParksWiki?.name,x.name].filter(Boolean).some(v=>n(v).includes(target)||target.includes(n(v))))||null}
...
function nextReference(cat){const cards=[...document.querySelectorAll(".trip-item")],card=cards.find(c=>!["complete","skip"].includes(c.dataset.status));if(!card)return null;const id=card.dataset.catalogId||card.dataset.itemId,entity=cat.entities?.[id]||Object.values(cat.entities||{}).find(x=>x.itineraryId===card.dataset.itemId),location=validLoc(entity);return location?{location,label:entity.name}:null}
...
function getReference(bar,cat,request,done){if(bar.dataset.ref!=="current"){bar.querySelector(".wait-tool-status").textContent="Distances use the next planned stop.";done(nextReference(cat));return}const cached=currentReference();if(cached){bar.querySelector(".wait-tool-status").textContent="Distances use recent phone location.";done(cached);return}if(!request){bar.querySelector(".wait-tool-status").textContent="Tap Current location to enable GPS; using next stop for now.";done(nextReference(cat));return}if(!navigator.geolocation){bar.querySelector(".wait-tool-status").textContent="GPS unavailable; using next stop.";done(nextReference(cat));return}bar.querySelector(".wait-tool-status").textContent="Getting phone location…";navigator.geolocation.getCurrentPosition(pos=>{const v={latitude:pos.coords.latitude,longitude:pos.coords.longitude,timestamp:Date.now()};sessionStorage.setItem("vaca-current-location",JSON.stringify(v));bar.querySelector(".wait-tool-status").textContent="Distances use current phone location.";done({location:v,label:"current location"})},()=>{bar.querySelector(".wait-tool-status").textContent="Location unavailable; using next stop.";done(nextReference(cat))},{enableHighAccuracy:true,timeout:10000,maximumAge:300000})}
