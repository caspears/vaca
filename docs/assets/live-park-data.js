...
function getReferences(bar,cat,panel,priority,request,done){const status=bar.querySelector(".wait-tool-status"),parks=[...new Map(priority.map(c=>[String(c.parkId),parkNameForId(panel,c.parkId)])).values()];const nextMap=()=>{const refs=new Map(parks.map(park=>[park,nextReference(cat,park)]));const available=[...refs].filter(([,ref])=>ref);if(parks.length===1&&available.length){const[,ref]=available[0];status.textContent=`Distances use next mapped stop: ${ref.label}.`}else if(available.length){status.textContent=`Distances use each park's next mapped stop.`}else status.textContent="No mapped itinerary reference is available.";done({mode:"next",refs})};if(bar.dataset.ref!=="current"){nextMap();return}const cached=currentReference();if(cached){status.textContent="Distances use recent phone location.";done({mode:"current",current:cached});return}if(!request){status.textContent="Tap Current location to enable GPS; temporarily using next mapped stops.";nextMap();return}if(!navigator.geolocation){status.textContent="GPS unavailable; using next mapped stops.";nextMap();return}status.textContent="Getting phone location…";navigator.geolocation.getCurrentPosition(pos=>{const v={latitude:pos.coords.latitude,longitude:pos.coords.longitude,timestamp:Date.now()};sessionStorage.setItem("vaca-current-location",JSON.stringify(v));status.textContent="Distances use current phone location.";done({mode:"current",current:{location:v,label:"current location",source:"gps"}})},()=>{status.textContent="Location unavailable; using next mapped stops.";nextMap()},{enableHighAccuracy:true,timeout:10000,maximumAge:300000})}
